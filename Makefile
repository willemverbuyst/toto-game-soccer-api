...
local:
	@echo "--> Starting $(NAME)"
	docker run $(LOCAL_OPTS) --name $(NAME) -p 8000:8000 -it $(IMAGE) /bin/bash
...
server:
	@echo "--> Starting $(NAME)"
	docker run $(LOCAL_OPTS) --name $(NAME) -p 8000:8000 -it $(IMAGE) uvicorn app.main:app --host 0.0.0.0 --port 8000
